...
function render() {
  ...
  $('#cgStatus').textContent = submitted ? 'הבחירה נשלחה לצלם ✓' : 'לחצו על תמונה כדי לבחור';
  ...
  $('#cgGrid').innerHTML = visible.map((photo) => {
    ...
    return `<button class="cg-photo ${isSelected ? 'selected' : ''}" data-id="${esc(photo.id)}" type="button" ${submitted ? 'aria-disabled="true"' : ''}>
      <img src="${esc(photo.image_url)}" alt="${esc(photo.filename || 'תמונה מהאירוע')}" loading="lazy">
      <span class="cg-check">${isSelected ? '✓' : '♡'}</span>
    </button>`;
  }).join('');
...
async function toggle(id) {
  ...
  if (wantsSelect) selected.add(id);
...
$('#cgSubmitBtn').addEventListener('click', async () => {
  ...
  try {
    const result = await call('submit_selection', {
    ...
    try {
      ...
      // The selection is already safely stored in Supabase even if email notification fails.
    }
    ...
    $('#cgSubmitMsg').textContent = error.message === 'Selection has already been submitted'
      ? 'הבחירה כבר נשלחה לצלם.'
